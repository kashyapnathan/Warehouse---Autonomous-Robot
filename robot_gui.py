...
class RobotEnv:
    ...
    def update(self):
        ...
        if self.program_state == 'exploration':
            self.robbie = exploration_state_machine(
                robbie=self.robbie, grid=self.grid)
    ...
    def update_motion(self):
        ...
        self.robbie.move_diff_drive(
            self.grid, self.robbie.vl, self.robbie.vr, self.robbie.dt)
        ...
        if self.program_state == 'exploration':
            ...
            if len(self.robbie.markers_found_or_picked) != len(set(self.robbie.markers_found_or_picked).union(set(marker_list))):
                self.robbie.markers_found_or_picked = list(
                    set(self.robbie.markers_found_or_picked).union(set(marker_list)))
                fname = self.grid.fname
                print(
                    f'{fname} found {len(self.robbie.markers_found_or_picked)}/{self.grid.LANDMARKS_TOTAL} markers')
        ...
        if self.testing:
            while True:
                try:
                    ...
                    print(
                        f'Exception in {self.map_name}: {e}! Thread terminated.')
                    break
